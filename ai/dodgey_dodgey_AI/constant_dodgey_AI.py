...
class DirectionalRegressor:
    def __init__(self, grid_size, stars = 4):
        self.coeffs = np.random.rand(grid_size) * [10/2**i for i in range(grid_size)]
        self.grid_size = grid_size
        self.stars = stars
        self.star_weights = np.random.rand(stars + 2) * [10/2**(stars + 2 - i) for i in range(stars + 2)]
        self.prev_pos = (0,0)
        self.curr_pos = (0,0)
        self.prev = '--'
        self.limit = 1
...
def main():
    GRIDSIZE = 7
    prime = DirectionalRegressor(GRIDSIZE, stars = 2)
    prime.coeffs = np.array([100,10,1,0,0,0,0])
    prime.star_weights = np.array([1000, 100, 10, 1, 0, 0, 0])
    ddf.play_game(ddf.GameGrid(GRIDSIZE), prime.decider, sleep = 2)
